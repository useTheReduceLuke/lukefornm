...
const FloatingListScrollBar = ({inRef}) => {
	...
	return (
		<div className={"flex flex-col h-full w-max absolute right-0 top-0"}>
			...
			<div className={"w-[2px] bg-white/75 relative h-10"}/>
			<motion.div
	...
	return (
		<span
			...
			<span ref={floatingRef}
			      className={"h-max max-h-full flex flex-col items-end text-end gap-0 px-2 py-2 md:py-3 opacity-100 md:opacity-50 hover:opacity-100 transition-all relative bg-gradient-to-r from-transparent to-transparent hover:to-black/40 overflow-y-auto"}>
				{
